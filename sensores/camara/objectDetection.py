...
class ObjectDetection:
    def __init__(self, display_width=640, display_height=480):
        # Guardar las dimensiones deseadas para la ventana de visulizacion	
    ...
    def show_frame(self, frame):
        # Convierte el frame de RGB a BGR si es necesario
    ...
    def start(self):
        ...
        try:
            while self._is_running:
                # Leer frame de la camara
                frame = self.read_frame()

                # Muestra el frame
                self.show_frame(frame)
        
                # Agregar un delay, 1ms
                cv2.waitKey(1)
...
#    camera = ObjectDetection(display_width=640, display_height=480)
